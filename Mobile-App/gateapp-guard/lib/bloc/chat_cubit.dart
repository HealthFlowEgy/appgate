...
import 'package:gateapp_guard/models/chat.dart';
import 'package:gateapp_guard/models/message.dart';
import 'package:gateapp_guard/models/message_attachment.dart';
...
class ChatCubit extends Cubit<ChatState> {
  ...
  initFetchChatMessages(Chat chat) async {
  ...
  initSendChatMessageText(Chat chat, String msgBody) async {
  ...
  unRegisterChatMessages() async {
    await _chatAddedStreamSubscription?.cancel();
    _chatAddedStreamSubscription = null;
    _messages.clear();
  }

  _handleFireChatAddedEvent(DatabaseEvent event) async {
  ...
  _handleFireInboxAddedEvent(DatabaseEvent event) async {
    if (event.snapshot.value != null) {
      try {
        Map resultMap = event.snapshot.value as Map;
        Map<String, dynamic> json = {};
  ...
  _handleFireInboxChangedEvent(DatabaseEvent event) async {
    if (event.snapshot.value != null) {
      try {
        Map resultMap = event.snapshot.value as Map;
        Map<String, dynamic> json = {};
  ...
  _notifyMessage(String recipientId) async {
    try {
      String chatRole = recipientId.contains(Constants.roleGuard)
          ? Constants.roleGuard
          : Constants.roleUser;
