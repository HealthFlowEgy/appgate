...
class LocalDataLayer {
  ...
  static const String guardProfileKey = "key_guard_profile";
  ...
  Future<GuardProfile?> getGuardProfileMe() async {
    if (_guardProfile == null) {
      await _initPref();
      _guardProfile = _sharedPreferences!.containsKey(guardProfileKey)
          ? GuardProfile.fromJson(
              jsonDecode(_sharedPreferences!.getString(guardProfileKey)!))
          : null;
    }
    if (_guardProfile != null) _guardProfile!.setup();
    return _guardProfile;
  }

  Future<void> setGuardProfileMe(GuardProfile guardProfile) async {
    _guardProfile = guardProfile;
    await _initPref();
    _sharedPreferences!
        .setString(guardProfileKey, jsonEncode(_guardProfile!.toJson()));
  }
  ...
  Future<bool> isLoggedIn() async {
    ...
    await getAuthenticationToken();
    await getUserMe();
    await getGuardProfileMe();
    return _guardProfile != null;
  }
