...
export class SaveComponent implements OnInit, IsEditable {
  ...
  parentServices: Array<Category> = [];
  metaScopeList = ['default'];
  editId = null;
  ...
  ngOnInit() {
    ...
    this.getParentServices().subscribe();

    this.getEditData();

    if (!this.editId) {
      this.scope = this.route.snapshot.queryParamMap.has('scope') ? this.route.snapshot.queryParamMap.get('scope') : 'service';
    }
  }

  getParentServices(): Observable<Array<Category>> {
    return this.client.parent(undefined).pipe(
      map(
        (response) => {
  ...
  saveCategory() {
    ...
    if (!this.editId) {
      formData.append('meta', JSON.stringify({ 'scope': this.scope }));
    }
